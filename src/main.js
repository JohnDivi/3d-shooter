...
async function main() {
    ...
    const vertShaderSample =
        ...
        void main() {
            ...
            oNormal = vec3((normalMatrix * vec4(aNormal, 0.0)).xyz);
            // normalize((uModelMatrix * vec4(aNormal, 1.0)).xyz);
    ...
    const fragShaderSample =
        ...
        void main() {
            ...
            vec3 halfVec = normalize(oCameraPosition + lightDirection);
            vec3 normal = normalize(oNormal);
            vec3 ambient = ambientVal * mainLight.colour;
            vec3 diffuse = mainLight.colour * max(dot(normal, lightDirection), 0.0);
            vec3 specular = specularVal * mainLight.colour * pow(max(dot(halfVec, normal), 0.0), nVal);

            vec4 textureColour = texture(uTexture, oUV);

            if (samplerExists == 1) {
                // diffuse *= mix(diffuseVal, textureColour.rgb, 0.7);
                fragColor = vec4(diffuse * textureColour.rgb + specular, alphaVal);
            } else {
                diffuse *= diffuseVal;
...
function drawScene(gl, deltaTime, state) {
    ...
    // sort objects by farness to camera
    ...
    sorted.map((object) => {
        gl.useProgram(object.programInfo.program);
        {
            ...
            let far = 500.0; // Far clipping plane
